...
import type { Props as ContentTodoProps, EditingProps } from "@/utils/types";
...
export default function ContentTodo({ decodeToken }: ContentTodoProps) {
  const contentStatus = "todo";
  const todos = useTodoStore((state) => state.todos?.filter((item: any) => item.status === "todo"));
  ...
  return (
    <div className={`card w-full shadow-xl bg-gray-700 rounded-md`} onDragOver={handleDragOverCard}>
      <div className="card-body px-[1.5rem] py-[1rem] text-white">
        ...
        <button className="btn btn-block text-white bg-gray-700" onClick={handleCreateTodo}>
